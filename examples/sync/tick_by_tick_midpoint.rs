...
//!
//! ```bash
//! cargo run --no-default-features --features sync --example tick_by_tick_midpoint
//! ```
...
fn main() {
    ...
    let ticks = client.tick_by_tick(&contract, 0).mid_point().expect("failed to get ticks");
