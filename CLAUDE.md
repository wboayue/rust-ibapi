...
12. **Integration tests verifying TWS wire format**: when an integration test exists to confirm "TWS accepts this encoding" (new fields, version-gated payloads, conditional orders, etc.), use `place_order` (not fire-and-forget `submit_order`), drain the subscription for ~3s, and fail if any non-warning `PlaceOrder::Message(Notice)` arrives. A single `is_some()` check on the first event passes even when TWS rejects the order — TWS sends an early `OrderStatus` before parse-error Notices. See [docs/integration-tests.md](docs/integration-tests.md#4-verifying-tws-accepts-a-wire-format-drain-events-and-assert-no-error-notice) for the canonical drain pattern
