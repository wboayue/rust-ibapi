...
- `OrderStatusKind` gains an `Unknown(String)` variant preserving unrecognized status strings, so a status this crate does not model no longer terminates order subscriptions (see the Fixed entry below). Like `Liquidity::Unknown` (#760), the enum stays deliberately exhaustive, so downstream exhaustive matches need a new arm — breaking. The variant carries a `String`, so `Copy` is gone (still `Clone`), `as_str()` returns `&str` instead of `&'static str`, and `is_active()` / `is_terminal()` take `&self` (both `false` for `Unknown`). Serialization shape is unchanged: the enum serializes as the plain status string in both directions, with `Unknown("X")` round-tripping as `"X"`. The nine known statuses parse exactly as before, matching stays exact and case-sensitive, and empty input is still `Error::Parse`. See `docs/migration-4.0.md` §9 (#774).
