...
- `MarketDataBuilder` moved from `market_data::builder` to `market_data::realtime`, alongside the sibling builders (`RealtimeBarsBuilder`, `MarketDepthBuilder`, `TickByTickBuilder`) it always belonged with; the `market_data::builder` module is gone. Only code naming the type — imports or signatures — breaks; `client.market_data(&contract)` call sites are unaffected. See `docs/migration-4.0.md` §7.
