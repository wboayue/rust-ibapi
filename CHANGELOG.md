...
- On the blocking client, every live subscription to a shared stream (`positions`, `account_updates`, `news_bulletins`, `open_orders`, ...) now receives every response, as on the async client. Previously subscriptions of the same type read one shared queue, so each response reached whichever of them read it first and the stream was split between them at random. A subscription could also start with items left over from before it was made: responses to an earlier request of the type, or a `ConnectionReset` from a socket drop while nothing was subscribed. Two concurrent one-shot calls of the same type (`server_time`, `managed_accounts`) each receive the reply; same-type calls with different arguments, such as `request_fa`, still cannot tell the replies apart. A shared subscription that was cancelled or failed by a reconnect no longer keeps collecting frames until its handle is dropped (#836).
...
- `OrderBuilder` setters for the integer-coded order enums that had no builder entry point: `trigger_method(TriggerMethod)`, `origin(OrderOrigin)`, `short_sale_slot(ShortSaleSlot)`, `designated_location(..)`, `volatility_type(VolatilityType)` and `reference_price_type(ReferencePriceType)`. `Order::trigger_method`, `origin`, `short_sale_slot`, `designated_location`, `volatility_type` and `reference_price_type` were public and reached the wire, but the fluent builder could not set any of them — the only path was constructing `Order` by hand, which is the surface the builder exists to replace. `volatility_type` had a private builder field whose value `build()` applied only when `volatility()` had also been called; the field had no setter, so it was always `None` and the guard never ran. The new setter is applied unconditionally (#832).
- `OrderBuilder::rule_80_a(Rule80A)` and `OrderBuilder::open_close(OrderOpenClose)`, the two string-typed order enums the #832 audit left out. Both are public `Order` fields that reach TWS (`Order.proto` `rule80A = 29` / `openClose = 68`) and had no builder entry point. That closes the audit: all ten enum-typed `Order` fields are now reachable from the builder — seven through same-named setters, `oca_type` through `.oca_group(group, OcaType)`, `action` through `.buy()` / `.sell()` / `.sell_short()` / `.sell_long()`, and `tif` through `.time_in_force()` and its named siblings (#833).
...
- `OrderBuilder::oca_group` takes `orders::OcaType` instead of a bare `i32`: `.oca_group("MyOCA", 1)` becomes `.oca_group("MyOCA", OcaType::CancelWithBlock)`. The enum was public and the `Order` field already typed; only the setter still took the raw code. See `docs/migration-4.0.md` §15 (#832).
- `orders::order_builder::auction_limit` drops its `auction_strategy` parameter: `auction_limit(action, quantity, price)`. See `docs/migration-4.0.md` §15 (#832).
...
- `orders::AuctionStrategy` and `Order::auction_strategy`. TWS's protobuf `Order` message has no auction-strategy field — IBKR's `Order.proto` does not declare one and the reference client's `createOrderProto` never sets one — so at the protobuf floor (server version 213) the field was written by `auction_limit` and dropped by `encode_order`, and no decoder ever produced one. It was unreachable in both directions, as `TickEFP` was in 3.0. See `docs/migration-4.0.md` §15 (#832).
- `orders::builder::AuctionType` and its `to_strategy()`. A public enum with no caller anywhere in the crate, whose codes (`1` / `2` / `4`) match no TWS auction field; it read as the setter enum for the auction gap above and was neither (#832).
- `#[repr(i32)]` and the explicit discriminants on `Liquidity`, `OcaType`, `OrderOrigin`, `ShortSaleSlot`, `VolatilityType`, `ReferencePriceType` and `TriggerMethod`. Nothing read them once each enum gained an `Unknown(i32)` payload variant — `variant as i32` no longer compiles either way — so the hand-written `From<T> for i32` is now the only source of truth, and each variant's wire code is documented in its rustdoc (#829).

### Fixed

- `historical_data(..).between(start, end)` returns the requested range. It sent the wall-clock span as an `N S` duration, which IBKR counts in trading seconds, so a one-day range of hourly bars returned about 3.7 regular-hours sessions ending at `end`, and any range over one day was rejected with code 321 (`S` durations are capped at 86400). It now requests `S` up to one day, `N+1 D` sessions up to 365, and calendar `Y` beyond, then drops bars timestamped outside `[start, end)` and sets `HistoricalData::start` / `end` to the requested range (#835).
- `Order::rule_80_a` and `Order::open_close` carrying an `Unknown("")` are omitted from the wire instead of sent as an empty string, matching `action` / `tif` and the reference client's `if (!Util.StringIsEmpty(..))`. A decode cannot produce an empty `Unknown` (`FromStr` rejects it), but the new setters put one a single call away (#833).
