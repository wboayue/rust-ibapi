...
- `OrderBuilder` setters for the order enums that had no builder entry point: `trigger_method(TriggerMethod)`, `origin(OrderOrigin)`, `short_sale_slot(ShortSaleSlot)`, `designated_location(..)`, `volatility_type(VolatilityType)` and `reference_price_type(ReferencePriceType)`. `Order::trigger_method`, `origin`, `short_sale_slot`, `designated_location`, `volatility_type` and `reference_price_type` were public and reached the wire, but the fluent builder could not set any of them — the only path was constructing `Order` by hand, which is the surface the builder exists to replace (#828).
...
- `OrderBuilder::oca_group` takes `orders::OcaType` instead of a bare `i32`: `.oca_group("MyOCA", 1)` becomes `.oca_group("MyOCA", OcaType::CancelWithBlock)`. The enum was public and the `Order` field already typed; only the setter still took the raw code. See `docs/migration-4.0.md` §15 (#828).
- `OrderBuilder::volatility_type` is applied whether or not `volatility()` was called. It was previously written only inside the `volatility` branch of `build()`, so a caller that set the type alone lost it silently (#828).
- `orders::order_builder::auction_limit` drops its `auction_strategy` parameter: `auction_limit(action, quantity, price)`. See `docs/migration-4.0.md` §15 (#828).
...
- `orders::builder::AuctionType` and its `to_strategy()`. A public enum with no caller anywhere in the crate, whose codes (`1` / `2` / `4`) match no TWS auction field; it read as the setter enum for the auction gap above and was neither (#828).
