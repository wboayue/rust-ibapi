...
- `OrderBuilder::rule_80_a(Rule80A)` and `OrderBuilder::open_close(OrderOpenClose)`, the two string-typed order enums the #832 audit left out. Both are public `Order` fields that reach TWS (`Order.proto` `rule80A = 29` / `openClose = 68`) and had no builder entry point. That closes the audit: all ten enum-typed `Order` fields are now reachable from the builder — eight through same-named setters, `action` through `.buy()` / `.sell()` / `.sell_short()` / `.sell_long()`, and `tif` through `.time_in_force()` and its named siblings (#833).
