...
- Every one-shot request now narrows the inbound frame to the message type it asked for before decoding it. Narrowing used to be opt-in — each domain hand-wrote a `decode_*_message` wrapper for it, and the sites that had no wrapper decoded whatever arrived on the shared channel. A foreign frame on a shared one-shot channel now surfaces as `Error::UnexpectedResponse` rather than being fed to the wrong payload decoder (#738).
