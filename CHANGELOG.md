...
- A notice whose protobuf `error_code` field is absent no longer fails every in-flight one-shot shared request (`server_time`, `managed_accounts`, `next_valid_order_id`, ...). The absent field decodes to code 0, which is outside the warning range, so the frame was treated as a request-less hard error. IB Gateway sends such code-less frames for informational notices, e.g. "Warning: Approaching max rate of 50 messages per second (42)". Code-0 frames now route as warnings: request-less ones reach `Client::notice_stream` without failing anything, and ones carrying a request id are delivered to that subscription as a notice instead of terminating it. Routing only — `Notice::category()` is code-range based and still reports code 0 as an error. An error frame that fails to decode at all also falls back to code 0 and takes the same path; that case now logs a warning instead of failing one-shots.
