...
- Log severity for unrouted notices and for notices received during the connection handshake is derived from `Notice::category()` instead of `is_warning()` plus per-code checks. Data advisories (317, 2188, 10xxx) log at `warn` on both paths; they logged at `error` or `warn` depending on the numeric band. Cancellation confirmations (202) log at `info` instead of `error`. The handshake path now grades like the unrouted path: warning-band and code 0 notices and 1101 at `warn`, 1100 and 1300 at `error`, where it logged all of them at `info` (#846).
