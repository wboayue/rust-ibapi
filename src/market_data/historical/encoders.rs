...
mod tests {
    ...
    fn test_encode_request_head_timestamp() {
        ...
        let results = encode_request_head_timestamp(request_id, &contract, what_to_show, use_rth);
    ...
    fn test_encode_request_historical_ticks() {
        ...
        let results = encode_request_historical_ticks(request_id, &contract, start, end, number_of_ticks, what_to_show, use_rth, ignore_size);
        ...
        match results {
            ...
            Err(err) => {
                assert!(false, "error encoding historical ticks request: {err}");
            }
        }
    }
}
