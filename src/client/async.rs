...
mod tests {
    ...
    async fn test_subscribe_to_group_events() {
        ...
        // Client::new is accessible here because we are in a child module
        let client = Client::new(connection_metadata, message_bus.clone()).expect("failed to create client");

        // The request ID will be 1 because next_request_id starts at 0?
        // Let's assume the client gets the next ID.
