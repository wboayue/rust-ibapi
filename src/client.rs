...
pub struct Client {
    ...
    client_id: i32, // ID of client.
     pub(crate) message_bus: Arc<Mutex<Box<dyn MessageBus>>>, // Wrap in Arc<Mutex<_>> to make it Send
...
unsafe impl Send for Client {} // Implement Send for Client explicitly since it contains non-Send fields
...
impl Client {
    ...
    pub fn connect(address: &str, client_id: i32) -> Result<Client, Error> {
        let message_bus = TcpMessageBus::connect(address)?;
        let boxed_bus: Box<dyn MessageBus> = Box::new(message_bus);

        let message_bus = Arc::new(Mutex::new(boxed_bus));
        
        Client::do_connect(client_id, message_bus)
    }

    fn do_connect(client_id: i32, message_bus: Arc<Mutex<Box<dyn MessageBus>>>) -> Result<Client, Error> {
        ...
        client.message_bus.lock().unwrap().process_messages(client.server_version)?;
    ...
    fn handshake(&mut self) -> Result<(), Error> {
        ...
        self.message_bus.lock().unwrap().write(&packet)?;

        let ack = self.message_bus.lock().unwrap().read_message();
    ...
    fn start_api(&mut self) -> Result<(), Error> {
        ...
        self.message_bus.lock().unwrap().write_message(prelude)?;
    ...
    pub fn receive_account_info(&mut self) -> Result<(), Error> {
        ...
        loop {
            let mut message = self.message_bus.lock().unwrap().read_message()?;
    ...
    #[cfg(test)]
    pub(crate) fn stubbed(message_bus: Arc<Mutex<Box<dyn MessageBus>>>, server_version: i32) -> Client {
        Client {
    ...
    pub(crate) fn send_message(&self, packet: RequestMessage) -> Result<(), Error> {
        self.message_bus.lock().unwrap().write_message(&packet)
    }
    ...
    pub(crate) fn send_request(&self, request_id: i32, message: RequestMessage) -> Result<ResponseIterator, Error> {
        ...
        self.message_bus.lock().unwrap().send_generic_message(request_id, &message)
    }
    ...
    pub(crate) fn send_durable_request(&self, request_id: i32, message: RequestMessage) -> Result<ResponseIterator, Error> {
        ...
        self.message_bus.lock().unwrap().send_durable_message(request_id, &message)
    }
    ...
    pub(crate) fn send_order(&self, order_id: i32, message: RequestMessage) -> Result<ResponseIterator, Error> {
        ...
        self.message_bus.lock().unwrap().send_order_message(order_id, &message)
    }
    ...
    pub(crate) fn request_next_order_id(&self, message: RequestMessage) -> Result<GlobalResponseIterator, Error> {
        self.message_bus.lock().unwrap().request_next_order_id(&message)
    }
    ...
    pub(crate) fn request_order_data(&self, message: RequestMessage) -> Result<GlobalResponseIterator, Error> {
        self.message_bus.lock().unwrap().request_open_orders(&message)
    }
    ...
    pub(crate) fn request_market_rule(&self, message: RequestMessage) -> Result<GlobalResponseIterator, Error> {
        self.message_bus.lock().unwrap().request_market_rule(&message)
    }
    ...
    pub(crate) fn request_positions(&self, message: RequestMessage) -> Result<GlobalResponseIterator, Error> {
        self.message_bus.lock().unwrap().request_positions(&message)
    }
    ...
    pub(crate) fn request_family_codes(&self, message: RequestMessage) -> Result<GlobalResponseIterator, Error> {
        self.message_bus.lock().unwrap().request_family_codes(&message)
    }
