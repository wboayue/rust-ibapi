...
mod tests {
    use crate::orders::builder::async_mock_client::mock::AsyncMockClient;
    use crate::orders::builder::{OrderBuilder, BracketOrderBuilder};
    use crate::orders::{Action, OrderState, PlaceOrder, OrderData, OrderUpdate, OrderStatus};
    ...
    impl<'a> crate::orders::builder::OrderBuilder<'a, AsyncMockClient> {
        ...
        pub async fn submit(self) -> Result<crate::orders::builder::OrderId, Error> {
            let client = self.client;
            let contract = self.contract;
            let order_id = client.next_order_id();
            let order = self.build()?;
            orders::submit_order(client, order_id, contract, &order).await?;
            Ok(crate::orders::builder::OrderId::new(order_id))
        }
    ...
    impl<'a> crate::orders::builder::BracketOrderBuilder<'a, AsyncMockClient> {
        pub async fn submit_all(self) -> Result<crate::orders::builder::BracketOrderIds, Error> {
            ...
            Ok(crate::orders::builder::BracketOrderIds::new(order_ids[0], order_ids[1], order_ids[2]))
        }
    }
