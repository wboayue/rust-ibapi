...
#[test]
fn test_consume_one_shot_processes_response() {
    let result = consume_one_shot(
...
#[test]
fn test_consume_one_shot_propagates_error() {
    ...
    let result: Result<IncomingMessages, Error> = consume_one_shot(
...
#[test]
fn test_consume_one_shot_delegates_closed_stream_to_on_none() {
    // on_none decides what a closed stream means: a default value...
    let result = consume_one_shot(None, |_| Ok(1), || Ok(0));
    assert!(matches!(result, Ok(0)));

    // ...or an error.
    let result: Result<i32, Error> = consume_one_shot(None, |_| Ok(1), || Err(Error::UnexpectedEndOfStream));
    assert!(matches!(result, Err(Error::UnexpectedEndOfStream)));
}
