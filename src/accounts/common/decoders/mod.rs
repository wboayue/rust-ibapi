...
pub(crate) fn decode_position(message: &mut ResponseMessage) -> Result<Position, Error> {
    message.decode_proto_or_text(decode_position_proto, |msg| {
        msg.skip(); // message type

        let message_version = msg.next_int()?; // message version
        ...
        position.contract.contract_id = msg.next_int()?;
        position.contract.symbol = Symbol::from(msg.next_string()?);
        ...
        position.contract.local_symbol = msg.next_string()?;
...
pub(crate) fn decode_position_multi(message: &mut ResponseMessage) -> Result<PositionMulti, Error> {
    message.decode_proto_or_text(decode_position_multi_proto, |msg| {
        msg.skip(); // message type
        ...
        position.position = msg.next_double()?;
        position.average_cost = msg.next_double()?;
        position.model_code = msg.next_string()?;
...
pub(crate) fn decode_pnl(server_version: i32, message: &mut ResponseMessage) -> Result<PnL, Error> {
    message.decode_proto_or_text(decode_pnl_proto, |msg| {
        msg.skip(); // message type
        msg.skip(); // request id

        let daily_pnl = msg.next_double()?;
        let unrealized_pnl = if server_version >= server_versions::UNREALIZED_PNL {
            Some(msg.next_double()?)
        } else {
            None
        };
...
pub(crate) fn decode_pnl_single(_server_version: i32, message: &mut ResponseMessage) -> Result<PnLSingle, Error> {
    message.decode_proto_or_text(decode_pnl_single_proto, |msg| {
        msg.skip(); // message type
        msg.skip(); // request id

        let position = msg.next_double()?;
        let daily_pnl = msg.next_double()?;
        let unrealized_pnl = msg.next_double()?;
        let realized_pnl = msg.next_double()?;
        let value = msg.next_double()?;
...
pub(crate) fn decode_account_value(message: &mut ResponseMessage) -> Result<AccountValue, Error> {
    message.decode_proto_or_text(decode_account_value_proto, |msg| {
        msg.skip(); // message type

        let message_version = msg.next_int()?;
        ...
        if message_version >= 2 {
            account_value.account = Some(msg.next_string()?);
        }
...
pub(crate) fn decode_account_portfolio_value(server_version: i32, message: &mut ResponseMessage) -> Result<AccountPortfolioValue, Error> {
    message.decode_proto_or_text(decode_account_portfolio_value_proto, |msg| {
        msg.skip(); // message type
        ...
        portfolio_value.position = msg.next_double()?;
        portfolio_value.market_price = msg.next_double()?;
        portfolio_value.market_value = msg.next_double()?;
        if message_version >= 3 {
            portfolio_value.average_cost = msg.next_double()?;
            portfolio_value.unrealized_pnl = msg.next_double()?;
            portfolio_value.realized_pnl = msg.next_double()?;
        }
        if message_version >= 4 {
            portfolio_value.account = Some(msg.next_string()?);
        }
        if message_version == 6 && server_version == 39 {
            portfolio_value.contract.primary_exchange = Exchange::from(msg.next_string()?)
        }
...
pub(crate) fn decode_account_multi_value(message: &mut ResponseMessage) -> Result<AccountMultiValue, Error> {
    message.decode_proto_or_text(decode_account_multi_value_proto, |msg| {
        msg.skip(); // message type
        msg.skip(); // message version
        msg.skip(); // request id
...
pub(crate) fn decode_account_update_message(server_version: i32, message: &mut ResponseMessage) -> Result<AccountUpdate, Error> {
