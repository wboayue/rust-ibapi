...
#[test]
fn notice_stream_receives_global_notices() {
    let client_id = ClientId::get();
    rate_limit();
    let client = Client::connect(GATEWAY, client_id.id()).expect("connection failed");

    let stream = client.notice_stream().expect("notice_stream failed");

    let notice = stream.next_timeout(Duration::from_secs(15));
    if let Some(notice) = notice {
        println!("received global notice: code={} message={}", notice.code, notice.message);
    } else {
        eprintln!("no global notice within 15s — quiet session, not a failure");
    }
}
