...
| PR 6 | News, scanner, WSH (request builders, sync/async test migration, sibling test files, encoder self-loop drops, dropped unused `MessageBus::request_messages` trait method) | Open |
...
5. ✅ **PR 5 — Market data** ([#500](https://github.com/wboayue/rust-ibapi/pull/500), merged). Request builders for historical (head_timestamp, historical_data, historical_ticks, histogram_data) and realtime (realtime_bars, tick_by_tick, market_depth, market_depth_exchanges, market_data). Migrated sync/async tests in both submodules to `assert_request<B>` body verification. Dropped 17 inline encoder self-loop tests (kept `test_encode_interval` — exercises the `OffsetDateTime: ToField` impl). Cancel builders dropped during /simplify pass (no test consumer); revisit when a `test_cancel_*` body assertion lands. Response builders deferred — market-data responses are text wire only and the existing inline literals exercise the production decoders.
6. ⏳ **PR 6 — News, scanner, WSH** (open). Request builders: news (providers, bulletins, cancel-bulletins, historical, article — `contract_news`/`broad_tape_news` reuse `MarketDataRequestBuilder`); scanner (parameters, subscription, cancel); wsh (metadata, event-data, cancels). Sync/async tests migrated to `assert_request<B>` body verification. Inline `mod tests` blocks moved to sibling `_tests.rs` files (`news/{sync,async}_tests.rs`, `scanner/{sync,async}_tests.rs`, `wsh/{sync,async}_tests.rs`, `wsh/common_tests.rs`). Self-loop encoder tests dropped from `{news,scanner,wsh}/common/encoders.rs` and from `wsh/mod.rs::common_tests`. Removed dead `MessageBus::request_messages` and `AsyncMessageBus::request_messages` trait methods (and their `MessageBusStub` impls) — the only consumers were the migrated sync tests, which now use the inherent `MessageBusStub::request_messages` via `assert_request<B>(&message_bus, …)`.
