...
### 13. One `TimeInForce`: `orders::builder::TimeInForce` is removed and the variants are spelled "till"
...
```rust,ignore
// 3.x
use ibapi::orders::builder::TimeInForce;
client.order(&contract).buy(100).limit(50.0).time_in_force(TimeInForce::GoodTillCancel).submit()?;
let order = Order { tif: ibapi::orders::TimeInForce::GoodTilCanceled, ..Default::default() };

// 4.0
use ibapi::orders::TimeInForce;
client.order(&contract).buy(100).limit(50.0).time_in_force(TimeInForce::GoodTillCanceled).submit()?;
let order = Order { tif: TimeInForce::GoodTillCanceled, ..Default::default() };
```

Rename the variants from either 3.x enum:

| `orders::builder::TimeInForce` (3.x) | `orders::TimeInForce` (3.x) | `orders::TimeInForce` (4.0) |
|---|---|---|
| `Day` | `Day` | `Day` |
| `GoodTillCancel` | `GoodTilCanceled` | `GoodTillCanceled` |
| `ImmediateOrCancel` | `ImmediateOrCancel` | `ImmediateOrCancel` |
| `GoodTillDate { date }` | `GoodTilDate` | `GoodTillDate`, with the date passed to `.good_till_date(date)` or `.good_till_time(date)` |
| `FillOrKill` | `FillOrKill` | `FillOrKill` |
| `GoodTillCrossing` | (none; parsed as `Day`) | `GoodTillCrossing` (new) |
| `DayTillCanceled` | `DayTilCanceled` | `DayTillCanceled` |
...
- **`decode_order` treats `action` as required.** An inbound order that omits it, or carries it empty, fails the decode with `Error::Parse`; previously an absent `action` read as `Buy` and an empty one hit `Action::from`'s `todo!()` and panicked. As with any decode error, any subscription that receives `OpenOrder` / `CompletedOrder` frames - `open_orders` and its `all_` / `auto_` variants, `completed_orders`, `order_update_stream`, `exercise_options`, a `place_order` subscription - yields the error and ends rather than skipping the frame. `rule_80_a` and `open_close` stay optional: absent or empty is `None`. `tif` is deliberately different: an absent or empty `tif` decodes as `Day` (per [§13](#13-one-timeinforce-ordersbuildertimeinforce-is-removed-and-the-variants-are-spelled-till)), because the reference client omits an empty `Tif` rather than sending one, so absence is an unset field with a known default; a missing side has no safe default.

```rust,ignore
// 3.x
let action = Action::from("SELL");                       // panicked on anything else
let rule: Option<Rule80A> = Rule80A::from("A");          // unknowns became None
let oc: Option<OrderOpenClose> = OrderOpenClose::from("O");
let code = OcaType::CancelWithBlock as i32;

// 4.0
let action: Action = "SELL".parse()?;                    // Err(Error::Parse) on anything else
let rule: Rule80A = "A".parse()?;                        // unknowns are Unknown(raw); "" is an error
let oc: OrderOpenClose = "O".parse()?;                   // same
let code = i32::from(OcaType::CancelWithBlock);          // `as i32` no longer compiles
```
...
- **`variant as i32` no longer compiles** on the seven integer-coded enums, because a payload variant removes the fieldless-enum cast. Use `i32::from(variant)` or `.into()`; `ToField` already does.
