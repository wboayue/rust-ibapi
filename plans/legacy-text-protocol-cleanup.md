...
|-------------------------------------------|--------------:|---------------:|------------------:|
| `accounts/common/decoders/`               |             3 |             11 |                 3 |
...
- `decode_realtime_bar`, `decode_trade_tick`, `decode_bid_ask_tick`, `decode_mid_point_tick` (gate 208), and `decode_market_depth`, `decode_market_depth_l2`, `decode_tick_price`, `decode_tick_size`, `decode_tick_string`, `decode_tick_generic`, `decode_tick_option_computation`, `decode_tick_request_parameters`, `decode_market_data_type` (gate 206) — proto-only since [#543](https://github.com/wboayue/rust-ibapi/pull/543); added 3 new tick-by-tick proto decoders (`decode_trade_tick_proto` / `decode_bid_ask_tick_proto` / `decode_mid_point_tick_proto`) over `proto::TickByTickData`; moved `decode_real_time_bar_proto` from historical → realtime; dropped `context.server_version` plumbing through the dispatcher; added `realtime/common/test_helpers.rs` for shared sync/async test fixtures. `decode_tick_efp` stays text-only (no server proto).
- `decode_position`, `decode_position_multi`, `decode_account_summary`, `decode_account_value`, `decode_account_portfolio_value`, `decode_account_multi_value` (gate 207), `decode_pnl`, `decode_pnl_single` (gate 210) — proto-only at floor 210; added `decode_account_update_time_proto` and converted `decode_account_update_time` (gate 207) which was previously text-only and broken under proto framing (rule 15 bug class); dropped `server_version` plumbing through `decode_account_update_message` + `dispatch_unsolicited_message`.
- `decode_family_codes` (gate 212 REST_MESSAGES_2) — proto-only at floor 213; receiver flipped `&mut ResponseMessage` → `&ResponseMessage` via `require_proto()`; sync/async fixtures migrated from `family_codes().encode_pipe()` text frames to `proto_response(IncomingMessages::FamilyCodes, ...encode_proto())`; added `_rejects_text_framing` regression test.
- `decode_server_time`, `decode_server_time_millis` (gate 213 REST_MESSAGES_3) — proto-only at floor 213; receiver flipped `&mut ResponseMessage` → `&ResponseMessage` via `require_proto()`; extracted `decode_server_time_proto` / `decode_server_time_millis_proto` helpers; sync/async fixtures migrated to `proto_response(IncomingMessages::CurrentTime{,InMillis}, ...encode_proto())`; deleted text-only `server_time_test_cases` table (`invalid_timestamp` / `overflow timestamp` scenarios structurally impossible under `Option<i64>` wire); marked `ResponseMessage::next_long` `#[allow(dead_code)]` for final-cleanup deletion.
- `decode_managed_accounts` (gate 213 REST_MESSAGES_3) — proto-only at floor 213; receiver flipped `&mut ResponseMessage` → `&ResponseMessage` via `require_proto()`; sync/async fixtures migrated from `managed_accounts().accounts(...).encode_pipe()` text frames to `proto_response(IncomingMessages::ManagedAccounts, ...encode_proto())`; deleted redundant `managed_accounts_test_cases` table (happy-path edge cases covered by existing `decode_managed_accounts_proto*` unit tests); added `_rejects_text_framing` regression test.
- `decode_market_depth_exchanges` (gate 213 REST_MESSAGES_3) — proto-only at floor 213; receiver flipped `&mut ResponseMessage` → `&ResponseMessage` via `require_proto()`; dropped `server_version` arg (legacy text branch swapped service-data-type encoding pre-SERVICE_DATA_TYPE); added `MktDepthExchangesResponse` + `DepthMarketDataDescriptionFields` builders in `src/testdata/builders/market_data.rs`; sync/async fixtures migrated from raw `"71|2|..."` text frames to `proto_response(IncomingMessages::MktDepthExchanges, ...encode_proto())`; added `_rejects_text_framing` regression test; marked `ResponseMessage::next_bool` `#[allow(dead_code)]` for final-cleanup deletion.
- `decode_display_group_updated` (gate 213 REST_MESSAGES_3) — proto-only at floor 213; collapsed the PR-A dual-format wrapper, receiver flipped `&mut ResponseMessage` → `&ResponseMessage` via `require_proto()`; added `DisplayGroupUpdatedResponse` builder in new `src/testdata/builders/display_groups.rs`; sync/async tests + decoder tests extracted to sibling `*_tests.rs` files (rule 8) and migrated from text frames to `proto_response(IncomingMessages::DisplayGroupUpdated, ...encode_proto())`; added `_rejects_text_framing` regression test.
